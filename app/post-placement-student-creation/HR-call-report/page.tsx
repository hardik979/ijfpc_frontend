...
function mapRow(r: SheetRow): Entry | null {
  ...
  const incoming = toNumber(r.INCOMING);
...
export default function HRCallReportPage() {
  ...
  const grouped = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([studentKey, arr]) => {
      ...
      const incoming = arr.reduce((s, r) => s + r.incoming, 0);
            ...
            {grouped.map((g) => (
              <div key={g.student} className={`${CARD_BASE}`}>
                ...
                <div className="flex items-center gap-4 text-white/85">
                  ...
                  {g.incoming > 0 && (
                    <div className="flex items-center gap-2">
                      <PhoneIncoming className="w-4 h-4 opacity-80" />
                      <span className="text-sm">{g.incoming} Incoming</span>
                    </div>
                  )}
                </div>
              </div>
